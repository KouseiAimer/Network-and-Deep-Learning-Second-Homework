...
def main():
    ...
    np.save(output_dir / "parta_split_indices.npy", indices)
    ...
    model = nn.models.Model_MLP(
        [train_images.shape[1], HIDDEN_DIM, 10],
        "ReLU",
    ...
    best_valid_acc = -1.0
    best_model_path = output_dir / "mlp_best_model.pickle"
    ...
    best_model = nn.models.Model_MLP()
    ...
    summary = {
        "seed": SEED,
        "model": f"784-{HIDDEN_DIM}-10 MLP with ReLU",
    ...
    with open(output_dir / "parta_results.json", "w", encoding="utf-8") as f:
    ...
    fig.tight_layout()
    fig.savefig(output_dir / "mlp_learning_curve.png", dpi=200)
    plt.close(fig)
